...
public class SugarcCRM {
    ...
    @Test(priority = 1, description = "Logo founded and take screenshot ", retryAnalyzer = Project.Pro.retryanalyzer.class )
    public void logoscreenshot() throws IOException {
    	
    	SelfHeal selfHeal = new SelfHeal();
        ...
        try {
            ...
            String primaryElement = "//img[contains(@alt, 'SugarCRM')]";
            String [] alternateElements = {"//a[@class='navbar-brand']/img[@alt='SugarCRM AU' and @class='logo'][1]"};
            
            // Find the logo element
//            WebElement logo = driver.findElement(By.xpath("//img[contains(@alt, 'SugarCRM')]"));
            WebElement applicationLogo = selfHeal.findElementwithRetry(driver,primaryElement,alternateElements);
            
            // Assert that the logo is displayed
            Assert.assertTrue(applicationLogo.isDisplayed());
            Reporter.log("Logo is displayed successfully.", true);
            
            // Take the screenshot of the logo
            File logoScreenshot = applicationLogo.getScreenshotAs(OutputType.FILE);
...
description = "Navigated to header solution and checked watch demo button is working",
retryAnalyzer = Project.Pro.retryanalyzer.class)
public void headercrmsolution() {
    try {
        Reporter.log("Starting test: Navigating to CRM Solution header.", true);
...
@Test(priority = 4, dependsOnMethods = {"acceptCookies"}, description = "Navigate to Industries and verifying contact as button is working", retryAnalyzer = Project.Pro.retryanalyzer.class )
public void headerIndustries() {
    try {
        Reporter.log("Starting test: Navigating to Industries header.", true);
...
@Test(priority = 5, dependsOnMethods = {"acceptCookies"}, retryAnalyzer = Project.Pro.retryanalyzer.class)
public void headerwhysugar() {
    try {
        Reporter.log("Starting test: Navigating to 'Why Sugar' header.", true);
...
@Test(priority = 6, dependsOnMethods = {"acceptCookies"} , description = "", retryAnalyzer = Project.Pro.retryanalyzer.class)
public void headerPartner() throws InterruptedException, IOException {
    try {
        Reporter.log("Starting test: Navigating to 'Partner' header.", true);
...
@Test(priority = 7, dependsOnMethods = {"acceptCookies"}, retryAnalyzer = Project.Pro.retryanalyzer.class)
public void Getdemo() throws InterruptedException {
    try {
        Reporter.log("Starting test: Clicking on 'Get a Demo' button.", true);
...
@Test(priority = 8, dependsOnMethods = {"Getdemo"} ,  description = "Enter user details", retryAnalyzer = Project.Pro.retryanalyzer.class)
public void enterUserDetails() {
    try {
        Reporter.log("Starting test: Entering user details.", true);
...
@Test(priority = 9, dependsOnMethods = {"enterUserDetails"}, description = "Enter Job details", retryAnalyzer = Project.Pro.retryanalyzer.class)
public void enterJobDetails() {
    SoftAssert softAssert = new SoftAssert();
...
@Test(priority = 10, dependsOnMethods = {"enterJobDetails"}, description = "Selecting Company Size And Industry", retryAnalyzer = Project.Pro.retryanalyzer.class)
public void selectCompanySizeAndIndustry() {
    SoftAssert softAssert = new SoftAssert();
